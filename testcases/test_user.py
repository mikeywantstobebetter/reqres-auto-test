...
def test_get_users_success(api_session, page):
...
def test_get_users_invalid_page(api_session):
...
def test_get_user_success(api_session, user_id):
    response = get_user(api_session, user_id)
    assert response.status_code == 200
    assert response.json().get("data")["id"] == user_id

def test_get_user_invalid_user_id(api_session):
...
def test_update_user_success(api_session, name, job):
